...
The last time it happened, I'd just migrated to Infomaniak. The photos came through fine. Everything else didn't. And I remember sitting there thinking: I did this to myself by trusting a platform that was never designed for me to leave.
...
There's already a standard for this: [XMP sidecars](https://en.wikipedia.org/wiki/Extensible_Metadata_Platform) (ISO 16684). Plain files, sitting next to your photos. Lightroom reads them. Darktable reads them. ExifTool reads them. They'll be readable long after any particular app is gone.
...
So I did. That became **OuEstCharlie**.

---

## The AI angle that actually matters

There's an irony in the current photo app landscape: at the exact moment AI assistants became genuinely useful, photo libraries became *more* siloed from them, not less.
...
**Woof** is the core of OuEstCharlie: a local MCP server you connect to any MCP-compatible AI assistant. It indexes your library, writes metadata to XMP sidecars (never touching your originals), and serves a full gallery — grid, carousel, full screen — inline in your conversation.
